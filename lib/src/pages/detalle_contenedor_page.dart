...
class DetalleContenedorPage extends StatelessWidget {
    ...
    return Scaffold(
        appBar: AppBar(
          title: StreamBuilder<bool>(
            ...
            builder: (_, snapshot) {
              if (snapshot.data!)
                return Text('Seleccionar cuys',
                    style: TextStyle(color: Colors.black45));
              else
                return Text('Detalles', style: TextStyle(color: Colors.black));
            },
          ),
            ...
            StreamBuilder<bool>(
                ...
                builder: (_, snapshot) {
                  if (!snapshot.data!)
                    return IconButton(
                        onPressed: () =>
                            _informacion(context, cuyBloc, contenedor),
          ...
          children: [
            StreamBuilder(
              ...
              builder: (BuildContext context,
                  AsyncSnapshot<List<CuyModel>> snapshot) {
                if (!snapshot.hasData)
                  return Center(child: CircularProgressIndicator());
                else {
                  if (snapshot.data!.isEmpty)
                    return Center(child: Text('No hay cuys'));
                  else {
                    final cuyList = snapshot.data!;
                    _cuyItemKeyList = List.generate(
                        cuyList.length, (index) => GlobalKey<CuyItemState>());
              ...
              return FlowMenu(
                children: [
                  ...
                  FloatingActionButton(
                    ...
                    tooltip: 'Agregar Nacimiento',
                    child: Icon(
                      Icons.auto_awesome,
                      size: 40,
                    ),
                    onPressed: () => Navigator.pushNamed(
                            context, 'nacimientoContenedor',
                  ...
                  FloatingActionButton.extended(
                    ...
                    onPressed: () {
                      ...
                      if (listSeleccionado.isNotEmpty) {
                        Navigator.pushNamed(context, 'mover',
                                arguments: listSeleccionado)
                            .then((value) {
                          cuyBloc.cargarCuysContenedor(contenedor.id);
                          cuyBloc.changeSeleccionar(false);
                        });
  ...
  void _informacion(
      BuildContext context, CuyBloc bloc, ContenedorModel contenedor) {
    showDialog(
        context: context,
        builder: (context) {
          return AlertDialog(
            ...
            builder: (context, snapshot) {
              return Row(
                children: [
                  ...
                  IconButton(
                      onPressed: () => bloc.incrementar(snapshot.data! + 1),
                      icon: Icon(Icons.add)),
                ],
              );
            },
